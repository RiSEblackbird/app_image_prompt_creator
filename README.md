...
4. **生成**: 「生成」ボタンで出力欄に表示、「生成とコピー」で生成完了後に全文をクリップボードへ
...
### movie プリセット
JSON形式で `video_style` を定義します。プリセットによっては `content_flags_defaults` と `direction_constraints_defaults` も同時に持ち、選択した瞬間に動画向けの既定パラメータを UI へ流し込めます。TV番組系（ニュース、旅番組、ドキュメンタリー等）、映画系（歴史超大作、青春映画、ファンタジー等）、報道・現地リポート、ロケ・体験番組など多数のカテゴリを選べます。添付画像の世界観を動画へ展開したい場合は、プリセット一覧ではなく独立トグル `添付画像に写る世界についての動画` を使い、選択中 `video_style.description` の先頭へ要件を前置きします。
...
2. **総尺を設定**:
   - **総尺決定=固定（指定した総尺）**: 10〜30秒（5秒刻み）から選択した値をそのまま使います
   - **総尺決定=モデル提案**: LLMが 8.0〜24.0秒 の範囲で `total_duration_sec` と各カットの `duration_sec` を提案します
3. **尺配分（総尺固定かつカット数指定時）**:
...
4. **カット生成方法を選択**:
   ...
   - **「カット数をLLM自動決定」**: ONにするとLLMがプロンプト内容からカット数(目安:3〜6)を推定し、各カットの `duration_sec` を提案します。総尺決定が固定なら **総尺（秒）はUIの選択値を厳守**し、総尺決定がモデル提案なら **総尺もLLMが提案**します。
5. **各カットを編集**: 説明、開始時刻、尺、カメラワークを設定
   - **開始時刻と尺は常に連動**します。開始時刻の変更は直前カットの尺に反映され、以降の開始時刻は累積で自動再計算されます。総尺は最後のカットの尺で吸収します（失敗する変更は警告して取り消します）。
...
- 総尺決定が固定でカット数/総尺を手動指定している場合、**尺配分を「均等（固定）/ 内容に応じて（LLM配分）」から選択**できます
- 総尺決定をモデル提案にすると、LLMは `total_duration_sec` と各カットの `duration_sec` をまとめて返す必要があります。`total_duration_sec` 欠落や、`duration_sec` 合計との不一致はエラーとして検知され、均等割当へは黙ってフォールバックしません。
- **描写可能性優先**: 各カットは「その秒数内で映像として描写できる内容」を最優先し、過密な脚本・段取り・説明過多は省略されることがあります（核心の意味は極力保持します）
- 生成後、各カットを手動で編集できます
- Soraの入力上限（約2000文字）を考慮し、LLMに「JSON全体を1900文字以内に収め、必要なら各カットの description だけを簡潔化する」指示を付与しています（メタ情報やカメラ指定は維持）
- 固定尺を持つテンプレート（例: `画像スタート（呪縛解除）`）では、総尺モデル提案モードは使えません。固定尺と可変総尺を同時に許すと、残り尺の契約が曖昧になるためです。
...
> **「現在のプロンプトから生成(LLM)」とテンプレートの関係**: 「テンプレートから初期化」と「現在のプロンプトから生成(LLM)」のどちらを使っても、テンプレートに定義された固定尺カット（例: 画像スタートの 0.3 秒プレースホルダー）は自動的に先頭へ挿入されます。LLM はその固定カット分を除いた残り尺・残りカット数でシーンを生成します。
...
- 「カット数をLLM自動決定」をONにすると、カット数(目安:3〜6)をLLMが推定し、`duration_sec` 付きで返します。総尺決定が固定なら **総尺（秒）はUIの選択値を厳守**し、総尺決定がモデル提案なら **8.0〜24.0秒の範囲で `total_duration_sec` も返します**。
...
```
→ 各カットに `duration_sec` を付けるか、ストーリーボード生成時に **尺配分を「内容に応じて（LLM配分）」** にして再生成してください（または自動構成をON）。
...
### プロンプトからの自動検出
- 出力欄に `@...` が含まれている場合、それをキャラクターIDとして検出し、LLMにキャラクター情報を渡します。
- 未登録のIDが見つかると「不足キャラクターの登録」ダイアログが開き、名前（必須）と3人称（任意）を入力して `sora_characters.yaml` に追記するか、「登録せず続行」を選べます。キャンセルすると生成を中断します。
- **全カットに登場するわけではありません**: LLMがカット内容に応じて、キャラクターが登場すべきかを判断し、適切なカットでのみIDを言及します。
- 正例: `@example.character1 深夜の高速道路で追跡劇` → キャラクターが登場するカットでのみ `@example.character1` が言及されます。
- 負例: `foo@bar.com` のようなメールアドレスは検出対象外（@直前が英数字のため）。
- **テキストへの埋め込み方針**: キャラクターはパラメータではなく名称として本文に明示し、`@id` の前後に必ず半角スペースを置きます。三人称よりもIDを優先します。  
  - 正例: `@ex.abc は驚いて飛び上がり、彼が持っていたジュースを落とす。`  
  - 負例: `彼は驚いて飛び上がり...`（ID未記載）/ `周囲に@ex.abcがいる`（IDにスペースなし）
...
| モデル | API |
|--------|-----|
| gpt-5.4 | Responses API（デフォルト） |
| gpt-4o | Chat Completions |
| gpt-4o-mini | Chat Completions |

画面右上のドロップダウンでモデルを即時切替できます。gpt-5系（gpt-5.4 / gpt-5.1）は Responses API を使用するため、temperature の代わりにシステムプロンプトで強弱を指示します。
