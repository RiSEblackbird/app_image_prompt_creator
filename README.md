...
- **英語出力ポリシー（アレンジ系）**: このタブの機能は LLMへ英語出力を要求します。混入した日本語の代表語は英訳へ置換します（通常生成の LLMモードのみ、別途「LLM生成言語」で日本語出力を選択可能）。
...
- **出力言語**: 通常生成の LLMモードのみ、UI の「LLM生成言語」で英語/日本語を切り替え可能です（Midjourney向けには英語推奨）。アレンジ/文字数調整/動画用整形/カオスミックスは引き続き英語出力前提でプロンプトを組み立てます。
