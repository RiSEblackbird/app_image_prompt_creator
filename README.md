...
- **末尾プリセット媒体切替**: 固定文セットを `image` / `movie` から選び、静止画/映像向けの差分を付けられる
 ...
 - **動画用プロンプト整形(JSON)**: 既に生成されたテキストプロンプトを、Sora2向けの「世界観JSON」フォーマットに整形します（LLMは使わずフォーマット変換のみ）
...
### 動画用プロンプト整形(JSON)
- 出力欄左下付近の「動画用に整形(JSON)」ボタンを押すと、現在の出力テキストから次のような変換を行います。
  - メインテキスト（末尾JSONと `--ar` 等のオプションを除いた部分）を `world_description` として JSON ラップ
  - JSON構造例:
    ```json
    {
      "world_description": {
    ...
    ```
  - 末尾の `{"video_style": ...}` JSON があればそのまま後ろに残し、さらにその後ろに `--ar` などのMidjourneyオプションを維持します。
- ねらい:
  - 元々は「脈絡のない短文の羅列」が動画モデルで「シーンの時系列」として解釈されがちでしたが、この整形により**世界観を説明する一つのJSON**として扱われやすくなります。
  - LLMは使わず、単純にテキストをJSON構造へマッピングするだけなので、挙動は決定的です。
