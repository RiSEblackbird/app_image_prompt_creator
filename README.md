...
- **プリセット**: `app_image_prompt_creator/arrange_presets.yaml` から読込（例: ファンタジー、サイバーパンク、和風、浮世絵、ゴシック、アール・デコ、アール・ヌーヴォー、アニメ、漫画、水彩画、ノワール、ヴェイパーウェーブ、パステル 等）
  ...
  - Tk版・PySide6版ともに同じ `arrange_presets.yaml` を参照します。
- **強度スライダー（0-3）**
...
補足:
- `arrange_presets.yaml` を編集して保存すると、Tk版は次回起動時に、PySide6版はアプリ起動中でも内部のアレンジプリセット定義を再読み込みします（将来のプリセットUI追加に備えてホットリロード対応済み）。
