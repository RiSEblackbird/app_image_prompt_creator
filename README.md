...
- **カオスミックス（LLM）**: 既存プロンプトを「1つの場面」に詰め込み、断片を無理やり同時発生させる情景文を生成
...
## カオスミックス機能（LLM）
- **ボタン位置**: 右ペイン「LLM アレンジ」タブの下部にある3つ目のボタン「カオスミックス」。
- **挙動**:
  - 現在のメインプロンプト（末尾JSONと `--options` を除いた本体）を文単位で分解し、LLMへ「1つの物理的な場面に全要素を詰め込む」指示を送ります。
  - 結果は1段落の英語で返り、`{"video_style": ...}` や `content_flags`、`--ar` 等のオプションは自動復元された上で全文コピーされます。
  - `文字数目標`／`上限`の設定はアレンジと共用で、上限を指定するとLLMに「〇〇文字以内」の制約が指示されます。
