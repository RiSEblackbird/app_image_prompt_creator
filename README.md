...
- **LLM生成言語切替**: 通常生成の LLMモードで、生成結果の言語を英語/日本語から選択可能（既定は英語）
- **LLM整形言語切替**: 動画用LLM改良（世界観整形/ストーリー構築/カオスミックス）と LLMアレンジ/文字数調整でも、英語/日本語の出力をワンタッチで切り替え可能（既定は英語）
...
- **動画用プロンプト整形(JSON)**: メインテキストをSora2向けの世界観JSONへ整形。LLMなしの「JSONデータ化」に加え、LLM改良の「世界観整形」「ストーリー構築」を用意し、各処理完了後は全文を自動コピー
  - **スタイル反映オプション**: LLM改良時にチェックを入れると、選択中の `video_style`（カメラ・照明・雰囲気など）だけでなく、末尾2 (`content_flags` JSON) もLLMへ伝え、スタイル定義と字幕/テロップ/音声指定に沿うように描写内容を補正します
...
2. **行数と生成方法の設定**
   ...
   - `LLM生成` のときは「LLM生成言語」で、生成されるフラグメント（1行ごとの短文）の言語を **英語 / 日本語** から選べます（既定: 英語）。
...
- 例3（動画用）: `movie` を選んで `{"video_style": ...}` を末尾に付与 → 通常どおり文を生成 → 左側の「動画用に整形(JSON)」ボタンを押す
  - 必要に応じて「スタイル反映」チェックをONにし、選択中の `video_style`（サスペンス風、ドローン撮影など）と末尾2(`content_flags`)の指定を世界観の描写自体にも反映させることができます。
...
- **報道・現地リポートカテゴリ (movie内)**: `section_sub_tv_field_report` を追加し、スタジオと屋外リポーターがリアルタイムで掛け合う 10 種のプリセット（例: `tv_field_breaking_news`, `tv_disaster_frontline`）を選択可能です。`camera` / `editing` / `sound_design` で「歩き回るカメラ」「ピクチャー・イン・ピクチャー」「環境音入り中継音声」などを事前に固定できます。既存の `tv_news_studio` / `tv_morning_show` も連続カメラワークと現地リポートの動きを強調する記述へ更新しました。
- **ロケ／体験・旅番組カテゴリリビルド (movie内)**: `section_sub_tv_travel` を全10パターンで刷新し、現地に入り込む体験型プリセット（`tv_street_food_crawl`, `tv_mountain_switchback_diary`, `tv_aurora_hunt_expedition` 等）を収録。各プリセットは `camera` や `sound_design` に「屋台を縫うハンドヘルド」「ライドシェア POV」「波や風の環境音」などを明記し、常に動きと臨場感が生成されるようにしています。
- **歴史／ファンタジー／ディザスター拡張 (movie内)**: `section_sub_movie_features_epic` に 10 プリセットを追加し、**具体的なモチーフを含まない抽象的な撮影・編集スタイル**（例: `movie_epic_mass_battle`, `movie_disaster_cascade`, `movie_fantasy_realm_war`）を収録しました。大規模会戦・攻城戦・海戦・空中戦・多段災害・レスキューリレー・カウントダウン・異世界戦争・空中追跡・儀式クライマックスといった「壮絶な映画の描き方」を `camera` / `editing` / `sound_design` で規定し、メインプロンプト側で指定する被写体やモチーフと干渉しません。
- **爆発的な映像・音響カテゴリ**: `movie` → `映画系プリセット` 内に「爆発的な映像・音響・ハイテンポ展開特化プリセット」を追加し、`movie_sonic_warfare` や `movie_neon_rave_strike` など 15 種の高エネルギー動画スタイルを選べます。巨大音圧・高速カット・閃光照明・多チャンネルサラウンドといった要素を video_style の `sound_design` / `editing` / `lighting` として事前に宣言できます。
...
- 正例（報道リレー）: 緊迫した現地感を出したい場合に `tv_field_breaking_news` を選択 → `editing` が「rapid live cut-ins」となり、スタジオと街頭の切替・人の動き・現地音声が必ず描写される。
- 負例（報道リレー）: 静物撮影や止まった展示室に `tv_disaster_frontline` を付ける → 画面内で人・車両・サイレンが動く設定になるため齟齬が発生。静的テーマでは既存のドキュメンタリー/教育プリセットを用いる。
- 正例（体験旅）: 食べ歩き企画に `tv_street_food_crawl` を付ける → フードトラック間を走り回るカメラ、屋台の音、短いカットチェンジがJSONに含まれ、現場の熱気を確実に反映できる。
- 負例（体験旅）: 場所固定の静物・建造物紹介に `tv_jungle_river_raft` を付ける → リバーラフティングや歓声が描写され、静止した映像と矛盾。落ち着いた番組には `tv_documentary_series` など別プリセットを使う。
- 正例（歴史/ディザスター）: メインプロンプトに「中世の騎士団」を書き、末尾で `movie_epic_mass_battle` を選ぶ → 大群衆が衝突する撮影・編集スタイルが付与され、具体的なモチーフはメイン側で自由に指定できる。
- 負例（歴史/ディザスター）: 小規模室内劇に `movie_epic_siege_assault` を付ける → 攻城戦向けの多視点カメラ・音響が前提になるため不整合。インティメイトな舞台には `movie_coming_of_age` 等を選ぶ。
- 正例（爆発的プリセット）: 高エネルギーのPVを作りたい場合に `movie_sonic_warfare` を選択 → `sound_design` に「earth-shaking low frequency blasts」が含まれる JSON が末尾に付与され、映像・音響の激しさをモデルへ直接伝えられる。
- 負例（爆発的プリセット）: 穏やかなネイチャードキュメンタリーに `movie_neon_rave_strike` を付ける → サウンド面や照明指示が過剰になり、静的内容と矛盾。静穏シーンでは既存の `movie_nature_documentary` など落ち着いたプリセットを選択する。
...
- 左ペイン「スタイル・オプション」タブの `末尾2 (JSONフラグ)` グループでは、以下2段構成で映像メタ情報を JSON として末尾に付与できます。
  ...
  - 登場人物: `(なし)` / `1人以上` / `1`〜`4`人 / `とても多い`
  - 構成カット数: `(Auto)` / `1` / `2` / `3` / `4` / `5` / `6` / `とても多い`
  - 動画中の言語: `(Auto)` / `日本語` / `英語`
- 出力される JSON 例:
  ```json
...
- `person_count` は `1+`（1人以上）/ `1`〜`4`（整数）/ `many`（群衆・5人以上）で出力されます。UI で「とても多い」を選ぶと `person_count: "many"` が含まれ、群衆が画面を占める想定をモデルへ伝えられます。
...
- `planned_cuts` は「作品全体をおおよそ何カットで構成するか」の目安を表し、`1-6` または `many`（7カット以上の高速モンタージュ）を指定します。`(Auto)` 選択時は `planned_cuts` フィールド自体が省略され、モデルに任せる前提になります。
...
- 正例（群衆＋高速編集）: `登場人物` を「とても多い」、`構成カット数` を「とても多い」に設定 → 末尾JSONに `"person_present":true,"person_count":"many","planned_cuts":"many"` が含まれ、大人数のラッシュ編集を明示できる。
...
- 負例: 群衆シーンにもかかわらず `登場人物` を `(なし)` のままにする、または高速編集を想定しているのに「構成カット数」を `(Auto)` にすると、`person_count` や `planned_cuts` が出力されずモデル側で意図が伝わりません。人数や編集密度を明示したい場合は該当のプルダウンを必ず設定してください。
...
- 出力欄左下付近の「動画用に整形(JSON)」パネルからモードを選択します。すべてのモードで処理完了時に全文が自動コピーされます。
  ...
  - **スタイル反映（チェックボックス）**: LLM改良ボタンの横にある「スタイル反映」をONにすると、現在選択されている末尾プリセット（`{"video_style": ...}`）に加えて末尾2（`{"content_flags": ...}`）の中身もLLMへの指示に追加します。これにより、スタイル定義だけでなく字幕/テロップ/音声フラグなどの指定も生成される文章へ反映されるようになります。
  - **出力言語ドロップダウン**: 同じ行にある「出力言語」で、LLM改良の結果を英語 / 日本語から選べます（既定: 英語）。
    - 正例: `出力言語` を「日本語」にして「世界観整形」を実行 → `world_description.summary` が自然な日本語で出力され、Sora向けに日本語コンテンツを設計しやすくなる。
    - 負例: 日本語出力を期待しているのに `出力言語` を「英語」のままにしてしまう → 結果が英語となり、再度ボタンを押し直す手間が発生。必要に応じて言語を切り替えてから実行してください。
...
- **出力言語コンボ**: タブ上部の「出力言語」で、アレンジ/文字数調整どちらの結果も英語 / 日本語から選択可能（既定: 英語）。
  - 正例: 出力言語を「日本語」にして強度1のアレンジを実行 → 元のプロンプト構造を保ったまま、日本語での納品原稿を得られる。
  - 負例: 日本語出力を求めているのに英語のままボタンを押す → 生成されたテキストが英語で再作業が必要。実行前に言語コンボを確認してください。
- **実行**: 「アレンジ(LLM)」/「アレンジしてコピー(LLM)」
- **既定出力言語**: コンボを変更しない場合は英語で出力されます。日本語プロンプトを得たい場合は実行前にコンボを切り替えてください。
...
- **挙動**:
  ...
  - 「スタイル反映」チェックをONにすると、`{"video_style": ...}` と `{"content_flags": ...}` の中身を LLM プロンプトに埋め込み、カオスシーンのカメラワークや照明・雰囲気に加えて字幕/テロップ/音声指定も守られるように誘導します。
...
- **出力言語**: 同じタブ上の「出力言語」で、調整結果を英語/日本語から選択可能（既定: 英語）。
...
- **出力言語**: 通常生成の LLMモードは「LLM生成言語」、動画用LLM改良とカオスミックスは「出力言語(動画)」、アレンジ/文字数調整は「出力言語(アレンジ)」で英語/日本語を切り替えられます（Midjourney向けには英語推奨。日本語出力が欲しい場合は各コンボを切り替えてください）。
...
## よくある質問（FAQ）
- **なぜ英語出力が推奨なのですか？**
  - Midjourney では英語の方が安定した挙動になりやすいため、既定値や多くのLLM機能は英語出力を前提に設計しています。通常生成の LLMモードでは UI の「LLM生成言語」で日本語出力も選択できますが、Midjourney 向けのプロンプトとしては英語出力を推奨します。
- **日本語のニュアンスは残せますか？**
  - アレンジ時にプリセットやガイダンスを和風寄せにする、固定文で和要素を補うなどで方向性を維持できます。通常生成の LLMモードで「LLM生成言語: 日本語」を選べば、下敷きとなるフラグメント自体を日本語で生成することもできます。
- **大量生成はできますか？**
