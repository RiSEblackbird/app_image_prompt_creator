...
# LLM 自動構成時のレンジ
STORYBOARD_AUTO_MIN_CUTS = 3
STORYBOARD_AUTO_MAX_CUTS = 6
STORYBOARD_AUTO_MIN_DURATION = 8.0
STORYBOARD_AUTO_MAX_DURATION = 24.0
STORYBOARD_AUTO_DEFAULT_DURATION = 12.0
...
BGM_GENRE_CHOICES = [
    ...
    ("サーカス", "circus"),
    ("軍楽隊", "marching_band"),
    ("過激インダストリアル", "harsh_industrial"),
    ("異界の儀式音楽", "alien_ritual"),
]
BGM_MOOD_CHOICES = [
    ("未指定", ""),
    ("静か", "calm"),
...
BGM_VOCAL_CHOICES = [
    ...
    ("デュエット", "duet"),
    ("ウィスパー", "whisper"),
    ("加工ボーカル", "processed_vocal"),
    ("詠唱", "chant"),
    ("ハミング", "humming"),
    ("スポークンワード", "spoken_word"),
    ("絶叫", "screamed"),
    ("機械音声", "synthetic_voice"),
    ("子ども合唱", "children_choir"),
    ("ロボット詠唱", "robotic_chant"),
    ("笑い声", "laughter"),
    ("グロウル", "growl"),
]
BGM_SYNC_CHOICES = [
    ...
    ("控えめに同期", "subtle"),
    ("ビートに同期", "beat_matched"),
    ("雰囲気重視で流す", "ambient_flow"),
    ("場面転換に合わせる", "transition_focus"),
    ("動きに合わせる", "motion_follow"),
    ("盛り上がりだけ同期", "crescendo_focus"),
    ("小節単位で同期", "bar_matched"),
    ("フレーズ単位で同期", "phrase_matched"),
    ("アクセントを強調", "punch_hit"),
    ("フレーム精度で同期", "frame_precise"),
    ("全カットで過剰同期", "overlocked"),
    ("カット無視で暴走", "deliberate_desync"),
    ("逆同期", "counter_sync"),
    ("視線移動まで同期", "gaze_synced"),
    ("唐突ブレイク連打", "break_hit"),
    ("秒単位で脈動同期", "pulse_locked"),
]
...
DIRECTION_CUT_DURATION_POLICY_CHOICES = [
    ...
    ("リズミカル", "rhythmic"),
    ("中盤長め", "mid_emphasis"),
    ("冒頭は短く導入後に伸ばす", "delayed_expansion"),
    ("クライマックス前に加速", "pre_climax_accel"),
    ("余韻重視", "lingering_ending"),
    ("対比重視", "contrastive"),
]
